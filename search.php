...
<html>
	<head>
		...
		<!-- <script type="text/javascript">
			...
			{
				setTimeout( function() {
				//Remove comments(fadeOut and fadeIn) to show transition
				$('#table')/*.fadeOut('slow')*/.load('refresh.php')/*.fadeIn('slow')*/;
				refresh();
								}, 2000);				//Refreshes every 2000 ms
			}
		</script> -->
		...
		<div class="container">
			<div class='control-buttons'>
				<form  method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">
					...
					<div class='control-buttons'>
						<fieldset class="col-xs-8 col-lg-offset-0" style="margin-top: 10px; padding-top: 10px; padding-bottom: 10px;">
							<button  type="submit"  name="submit" class="btn btn-info col-xs-6" > search </button>
			...
			<?php
				...
				$sql = "SELECT * FROM rail_form_detail where status !='approved'";
			...
			{	
				...
				if($date_from && $student_id)
				{
					...
					$sql = "SELECT * FROM rail_form_detail WHERE st_id ='$student_id' AND pass_from >'$date_from' and status != 'approved'";
					$result = $conn->query($sql);
				}
				...
				else if($name && $student_id && $date_from)
				{
					...
					$sql = "SELECT * FROM rail_form_detail where st_id= '$id' AND pass_from >'$date_from' and status != 'approved'";
					$result = $conn->query($sql);
				...
				else if($date_from)
				{
					$date_from  = $_POST['from_date'];
					echo "pass from  ". $date_from ;
					$sql = "SELECT * FROM rail_form_detail WHERE pass_from >'$date_from' and status != 'approved'";
					$result = $conn->query($sql);
				}
				...
				else if($name)
				{
					...
					$sql = "SELECT * FROM rail_form_detail where st_id= '$id' and status != 'approved' ";
					$result = $conn->query($sql);
				...
				else if($student_id)
				{
					...
					$sql = "SELECT * FROM rail_form_detail WHERE st_id ='$student_id' and status != 'approved'";
					$result = $conn->query($sql);
